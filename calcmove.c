...
Coordinate DetermineMove(BoardState *board, enum AiType aiType, int playerNum) {
	...
	for (int i = 0; i < playerCount; i++) {
		playerList[i] = malloc(maxTilesPerChar * sizeof(Coordinate));
		for (int j = 0; j < maxTilesPerChar; j++) {
			playerList[i][j].x = -1;
			playerList[i][j].y = -1;
		}
	}
	...
	for (int player = 0; player < playerCount; player++) { //iterate through players
		for (int i = 0; playerList[player][i].x != -1; i++) { //iterate through tiles
			...
			if (n = CheckAdjacencyRecursive(coord, 1, 0, board, 0)) { //check rightwards
				if (coord.x - 1 > 0)
					weights[coord.x - 1][coord.y] += (float)pow(weight, n);
				if (coord.x + n/* + 1*/ < xLeng/* - 1*/)
					weights[coord.x + n + 1][coord.y] += (float)pow(weight, n);
			}
			if (n = CheckAdjacencyRecursive(coord, 1, 1, board, 0)) { //check right-downwards
				if (coord.x - 1 >= 0 && coord.y - 1 >= 0)
					weights[coord.x - 1][coord.y - 1] += (float)pow(weight, n);
				if (coord.x + n/* + 1*/ < xLeng/* - 1*/ && coord.y + n/* + 1*/ < yLeng/* - 1*/)
					weights[coord.x + n + 1][coord.y + n + 1] += (float)pow(weight, n);
			}
			if (n = CheckAdjacencyRecursive(coord, 0, 1, board, 0)) { //check downwards
				if (coord.y - 1 >= 0)
					weights[coord.x][coord.y - 1] += (float)pow(weight, n);
				if (coord.y + n/* + 1*/ < yLeng/* - 1*/)
					weights[coord.x][coord.y + n + 1] += (float)pow(weight, n);
			}
	...
	for (int player = 0; player < playerCount; player++) { //iterate through players
		if (playerNum == player) {
			...
			//change the weight of tiles around each player tile
				for (int x = -1; x < 1; x++) {
					for (int y = -1; y < 1; y++) {
						weights[playerList[player][i].x + x][playerList[player][i].y + y] += allyDistWeight;
					}
				}
			}
		}
